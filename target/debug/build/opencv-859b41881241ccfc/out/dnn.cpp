...
void cv_dnn_Image2BlobParams_Image2BlobParams_const_ScalarR_const_SizeR_const_ScalarR_bool_int_DataLayout_ImagePaddingMode(const cv::Scalar* scalefactor, const cv::Size* size, const cv::Scalar* mean, bool swapRB, int ddepth, cv::dnn::DataLayout datalayout, cv::dnn::ImagePaddingMode mode, Result<cv::dnn::Image2BlobParams>* ocvrs_return) {
	try {
		cv::dnn::Image2BlobParams ret(*scalefactor, *size, *mean, swapRB, ddepth, datalayout, mode);
		Ok(ret, ocvrs_return);
	} OCVRS_CATCH(ocvrs_return);
}
